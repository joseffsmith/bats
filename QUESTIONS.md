...
- ~~**Stalemate / no-progress draw**~~ **Resolved** (2026-08-01): PLAN.md defines win only via HQ capture or rout, so two camping players could run forever. The rule chosen is a **day cap at the UI layer, not a turn cap in the engine**. `src/engine/systems/win.ts` is unchanged — HQ capture or rout remain the only ways to set `state.winner`, which the campaign workstream depends on. Instead:
  - the tie-break ladder the tournament harnesses already used was lifted into a pure query, `src/engine/queries/adjudicate.ts` (real winner → more HQ tiles → more HP-weighted unit cost by a margin > 1 → draw). `src/cli/round-robin.ts` and `src/cli/tournament.ts` now import it instead of keeping duplicate copies;
  - `src/renderer/adjudication.ts` holds the cap itself (`STALEMATE_DAY_CAP = 60`, day = ⌈turn/2⌉). Past day 60 both shells present the verdict — desktop via chrome.ts's winner overlay, mobile via the tray takeover — reading "Adjudicated: <side> prevails" or "Draw", with Play Again;
  - `state.winner` is **never forged**. The board is locked instead through `matchConcluded` predicates on the input controller and the AI driver, wired in `src/main.ts`;
  - **skirmish only.** `main.ts` passes `adjudicateStalemate: mission === null`, so campaign missions keep being scored by their own `defeat.dayLimit` (`src/campaign/tracker.ts`) and never see a second clock.
  Tests: `tests/adjudicate.test.ts` (the ladder) and `tests/stalemate.test.ts` (both shells, the campaign gate, the board lock).
